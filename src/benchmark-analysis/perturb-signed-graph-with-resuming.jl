#----------------------------------------------------------------
# Note that the perturbation process used in this script can detect if an already perturbed signed graph can be still perturbed more.
# For instance, the perturbation process is interrupted for some reasons before, it would be very convenient to resume the perturbation process from where the process is interrupted.
#----------------------------------------------------------------
...
# ==========================================
function get_remaining_indexes_for_internal_links(membership, source_ids, target_ids, weights)
    remaining_indxs = [];
...
# ==========================================
function get_remaining_indexes_for_external_links(membership, source_ids, target_ids, weights)
    remaining_indxs = [];
    for i in 1:size(source_ids,1)
       if membership[source_ids[i]+1] != membership[target_ids[i]+1] # outside cluster
            if weights[i]<0
                push!(remaining_indxs,i);
            end
       end
...
# ==========================================
function get_remaining_indexes_for_all_links(membership, source_ids, target_ids, weights)
    remaining_indxs_internal = get_remaining_indexes_for_internal_links(membership, source_ids, target_ids, weights);
    remaining_indxs_external = get_remaining_indexes_for_external_links(membership, source_ids, target_ids, weights);
    remaining_indxs = shuffle!(Array{Int32}(vcat(remaining_indxs_internal, remaining_indxs_external)));
    return remaining_indxs;
end
...
function perturb_signed_graph(out_folder, graph_filepath, membership_filepath, vars_filepath, constrs_filepath, bounds_filepath, nb_attemps_for_termination, target_link_type, n, cplex_threads)
    ...
    if(start)
        ...
        # ================================================================================
        # --------------------------------------
        # if the perturbed graph file does not exist
        # --------------------------------------
        ...
        # --------------------------------------
        # if the perturbed graph file does exist
        # --------------------------------------
        if isfile(pert_graph_filepath)
            global w_perturbed = readdlm(pert_graph_filepath,'\t', skipstart=1)[:,3];
            global indxs = Int32.(findall(w .!= w_perturbed));
            println(indxs)
            global delta = size(indxs,1);
            println("delta: ",delta)
        end
        # ================================================================================

        global remaining_indxs = get_remaining_indexes_for_internal_links(membership, source_ids, target_ids, w_perturbed);
        if target_link_type == "external"
            global remaining_indxs = get_remaining_indexes_for_external_links(membership, source_ids, target_ids, w_perturbed);
        elseif target_link_type == "all"
             global remaining_indxs = get_remaining_indexes_for_all_links(membership, source_ids, target_ids, w_perturbed);
        end
        ...
        while size(remaining_indxs,1)>0 && nb_attemps>0
            ...
            println(string("!!!!!!! time BEFORE checking the optimality: ", Dates.format(now(), "HH:MM:SS")))
            ...
            println(string("!!!!!!! time AFTER checking the optimality: ", Dates.format(now(), "HH:MM:SS")))
